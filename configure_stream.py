...
class region_class:
    ...
    def click_event(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            coordinate_label = input("Enter the label for the point: ")
            with open(f"coordinate_{coordinate_label}.json", "w") as f:
                json.dump({"x": x, "y": y}, f)
            print("This is the coordinates of the point: ", x, y)
...
cap = cv2.VideoCapture(r"demo_videos\video_0.mp4")

ret, image = cap.read()
image = cv2.resize(image, (1280, 720))

region_drawer = region_class(image)
region_points = region_drawer.get_image_coordinates()